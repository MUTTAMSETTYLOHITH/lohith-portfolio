        ...
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-2">
            {[
              ...
              [
                <span key="g" className="inline-flex items-center gap-1"><Github className="h-4 w-4"/> GitHub</span>,
                "https://github.com/MUTTAMSETTYLOHITH"
              ],
            ].map(([label, href]) => (
              <a
                key={(typeof label === "string" ? label : (label as any).key) as string}
                href={href as string}
                target={String(href).startsWith("http") || String(href).endsWith(".pdf") ? "_blank" : undefined}
                rel={String(href).startsWith("http") || String(href).endsWith(".pdf") ? "noopener noreferrer" : undefined}
